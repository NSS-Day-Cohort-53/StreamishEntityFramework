    ...
    public class VideoRepository : BaseRepository, IVideoRepository
    {
        ...
        public Video GetById(int id)
        {
            using (var conn = Connection)
            {
                ...
                {
                    cmd.CommandText = @"
                          SELECT v.Title, v.Description, v.Url, v.DateCreated VideoDate, v.UserProfileId,
                                 u.Name, u.Email, u.ImageUrl, u.DateCreated UserDate
                          FROM Video v
                            JOIN UserProfile u ON u.Id = v.UserProfileId
                          WHERE v.Id = @Id";
        ...
        public List<Video> SearchByDate(DateTime dateTime)
        {
            using (var conn = Connection)
            {
                ...
                {
                    var sql = @"
                        SELECT v.Id, v.Title, v.Description, v.Url, v.DateCreated AS VideoDateCreated, v.UserProfileId, up.Name, up.Email, up.DateCreated AS UserProfileDateCreated, up.ImageUrl AS UserProfileImageUrl
                        FROM Video v 
                            JOIN UserProfile up ON v.UserProfileId = up.Id
